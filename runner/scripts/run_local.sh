...
LOCAL_SESSIONS_PATH=${HOME}/pearcode-sessions
CONTAINER_SESSIONS_PATH=/pearcode-sessions
...
docker run --rm \
    ...
    -v $LOCAL_SESSIONS_PATH:$CONTAINER_SESSIONS_PATH \
    --name pearcode-runner \
    -p 8080:8080 \
    --env LISTENING_PORT=$LISTENING_PORT \
    --env CORS_ALLOWED_DOMAINS=$CORS_ALLOWED_DOMAINS \
    --env LOCAL_SESSIONS_PATH=$LOCAL_SESSIONS_PATH \
    --env CONTAINER_SESSIONS_PATH=$CONTAINER_SESSIONS_PATH \
